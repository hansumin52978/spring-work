<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <script>
       ...
       //fetch('url', {요청 관련 정보를 담은 객체(GET방식에서는 따로 전달 안함.)})
        ...
        .then(res => {
            ...
            //콜백 방식으로 전달합니다. (function(rest) -> 화살표 함수로 간단히 표현)
        ...
        .then(data => {
            console.log('data: ', data); //최종적으로 데이터를 확인하는 곳
        })
        ...
        */
        ...
        //화살표 함수 내의 코드가 한줄이고, 그것이 return이라면 괄호와 return 생략 가능
        fetch('/myweb/user/id/' + userId)
        .then(res => res.text()) //요청 완료 후 응답 정보에서 텍스트 데이터가 달린 Promise 반환.
    ...
    document.getElementById('joinBtn').onclick = function() {
        if(idFlag && pwFlag) { //&& and 연산자 둘다 true일 때
        ...
        /*아이디 형식 검사 스크립트*/
        var id = document.getElementById("userId"); //아이디 입력창
        id.onkeyup = function() { //onkeyup은 키를 눌렀다가 뗐을 때 발생하는 이벤트
            /*
                자바스크립트의 정규표현식 입니다
            ...
            */
            ...
            // test(문자열) -> 정규표현식의 규칙에 어긋나지 않는 문장라면 true, 어긋난다면 false
